        ...
        <div class="user-characters">
            <div class="row">
                @foreach($characters as $character)
                    <div class="col-lg-2 col-sm-3 col-6 mb-3 user-character category-all category-{{ $character->character_category_id ? : 0 }} {{ isset($selected) && in_array($character->id, $selected) ? 'category-selected' : '' }} {{ (isset($selected) && in_array($character->id, $selected)) || ($character->isAvailable && !$character->is_disabled) ? '' : 'select-disabled' }}" data-id="{{ $character->id }}">
                        <div class="text-center character-item {{ (isset($selected) && in_array($character->id, $selected)) || ($character->isAvailable && !$character->is_disabled) ? '' : 'disabled' }}" @if(!(isset($selected) && in_array($character->id, $selected)) && (!$character->isAvailable || $character->is_disabled)) data-toggle="tooltip" title="{{ $character->is_disabled ? 'This character is disabled.' : ($character->trade_id ? 'This character is in a trade.' : 'This character has an active design update.') }}" @endif>
                            <div class="mb-1">
