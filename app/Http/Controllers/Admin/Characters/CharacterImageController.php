...
class CharacterImageController extends Controller
{
    ...
    */
    ...
    public function postNewImage(Request $request, CharacterManager $service, $slug)
    {
        ...
        $data = $request->only(['image', 'thumbnail', 'x0', 'x1', 'y0', 'y1', 'use_cropper', 'artist_url', 'artist_alias', 'designer_url', 'designer_alias', 'species_id', 'subtype_id', 'subtype_id_2', 'rarity_id', 'feature_id', 'feature_data', 'is_valid', 'is_visible']);
        $this->character = Character::where('slug', $slug)->first();
        if(!$this->character) abort(404);
    ...
    {
        $data = $request->only(['species_id', 'subtype_id', 'subtype_id_2', 'rarity_id', 'feature_id', 'feature_data']);
        $image = CharacterImage::find($id);
        if(!$image) abort(404);
