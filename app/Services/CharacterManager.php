    ...
    public function createTransfer($data, $character, $user)
    {
        ...
        try {
            ...
            foreach($character->designUpdate as $update)
            {
                ...
                {
                   if(!(new DesignUpdateManager)->rejectRequest('Cancelled by '.$user->displayName.' in order to transfer character to another user', $update, $user, true, false)) throw new \Exception('Could not cancel pending request.');
                }
            }
